...
int main(){
	...
	double GMRES(double A[],double x0[],double b[],double x[],int m,int n,double epsilon);
	void VDMMatinput(double VDM[],double a[],int n);
	void MatCon(double Q[],double z[],int m,int n);
	
	int n=18,i,j,m,k=100,t=2;//t为步长，k为重启次数 
	m=t;
	double A[n*n],x0[n],x[n],b[n],a[n];
	double epsilon=0.000001,r;
